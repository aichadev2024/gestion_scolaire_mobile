...
class _PromoteurDashboardState extends State<PromoteurDashboard> {
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('ESPACE PROMOTEUR', style: AppTheme.mono(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.laterite, letterSpacing: 1.5)),
                          const SizedBox(height: 2),
                          Text('$prenom $nom', style: AppTheme.display(fontSize: 20, color: AppTheme.indigo)),
                ...
                if (_isLoading && _stats == null)
                  const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator(color: AppTheme.indigo)))
                else ...[
                  Text('Effectifs', style: AppTheme.display(fontSize: 16, color: AppTheme.indigo)),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(child: _statBox('Élèves', '${_stats?['totalEleves'] ?? 0}', Icons.groups_rounded, AppTheme.mil)),
                      const SizedBox(width: 10),
                      Expanded(child: _statBox('Enseignants', '${_stats?['totalEnseignants'] ?? 0}', Icons.school_rounded, AppTheme.laterite)),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(child: _statBox('Classes', '${_stats?['totalClasses'] ?? 0}', Icons.door_front_door_rounded, AppTheme.indigo)),
                      const SizedBox(width: 10),
                      Expanded(child: _statBox('Personnel', '${_stats?['totalPersonnel'] ?? 0}', Icons.badge_rounded, AppTheme.flagGreen)),
                    ],
                  ),
                  const SizedBox(height: 24),

                  Text('Finances réelles', style: AppTheme.display(fontSize: 16, color: AppTheme.indigo)),
                  const SizedBox(height: 12),
                  Container(
                    padding: const EdgeInsets.all(20),
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _ligneFinance('Frais attendus', '${_montant(_stats?['totalFraisAttendus'])} $devise', AppTheme.ink),
                        const Divider(height: 24),
                        _ligneFinance('Encaissé', '${_montant(_stats?['totalEncaisse'])} $devise', AppTheme.flagGreen),
                        const Divider(height: 24),
                        _ligneFinance('Solde restant', '${_montant(_stats?['soldeRestant'])} $devise', AppTheme.laterite),
                      ],
                    ),
                  ),
                  const SizedBox(height: 24),

                  Text('Comptes du personnel', style: AppTheme.display(fontSize: 16, color: AppTheme.indigo)),
                  const SizedBox(height: 4),
                  Text('Lecture seule — gérés par la direction', style: AppTheme.body(fontSize: 11, color: AppTheme.inkMuted)),
                  const SizedBox(height: 12),
                  if (_personnel.isEmpty)
                    Container(
                      padding: const EdgeInsets.all(20),
                      decoration: AppTheme.cardDecoration(),
                      child: Text('Aucun compte pour le moment.', style: AppTheme.body(color: AppTheme.inkMuted, fontSize: 13)),
                    )
                  else
  ...
  Widget _ligneFinance(String label, String valeur, Color color) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: AppTheme.body(fontSize: 13, color: AppTheme.inkMuted)),
        Text(valeur, style: AppTheme.display(fontSize: 16, fontWeight: FontWeight.bold, color: color)),
      ],
    );
  }
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('$prenom $nom'.trim().isEmpty ? (u['username'] ?? u['email'] ?? '—').toString() : '$prenom $nom',
                    style: AppTheme.body(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.ink)),
                Text(role, style: AppTheme.body(fontSize: 11, color: AppTheme.inkMuted)),
              ],
            ),
          ),
          Container(
            ...
            decoration: BoxDecoration(
              color: (estActif ? AppTheme.flagGreen : AppTheme.danger).withValues(alpha: 0.12),
              borderRadius: BorderRadius.circular(20),
            ),
            child: Text(
              estActif ? 'Actif' : 'Désactivé',
              style: AppTheme.body(fontSize: 10, fontWeight: FontWeight.bold, color: estActif ? AppTheme.flagGreen : AppTheme.danger),
            ),
          ),
        ],
      ),
